...
class MCU2Event extends Model{
    use HasFactory;

    public static function query($vars = [], $params = []){
        $db =
            DB::table("mcu2_event AS a")
            ->select(DB::raw("a.*,
                        b.title AS client_name, b.pic_name AS client_pic_name, b.pic_phone AS client_pic_phone, b.pic_email AS client_pic_email,
                        e.title AS mcu2_format_name,
                        sa.name AS status_name, sa.color AS status_color"))
            ->leftJoin("provider AS b", function($join){
                $join->on("a.client_id","=","b.id");
            })
            ->leftJoin("mcu2_format AS e", function($join){
                $join->on("a.mcu2_format_id","=","e.id");
            })
            ->leftJoin(DB::raw("(SELECT * FROM status_all WHERE tbl='mcu2_event') AS sa"), function($join){
        ...
        if(isset($vars['countPatient']) && $vars['countPatient']){
            $qPatient =
                ...
                ->select(DB::raw("a.mcu2_event_id,
                        ...
                        SUM(IF(a.status = 'sHalfFinished', 1, 0)) AS patient_halffinished,
                        SUM(IF(a.status = 'sFinished', 1, 0)) AS patient_finished"))
                ->whereRaw("a.delete_at IS NULL")
                ->groupBy("a.mcu2_event_id");
            $db = 
                $db->addSelect(DB::raw("f.patient_total, f.patient_scheduled, f.patient_presented, f.patient_canceled, f.patient_halffinished, f.patient_finished"))
                ->leftJoin(DB::raw("(".$qPatient->toSQL().") AS f"), function($join){
                    $join->on("a.id","=","f.mcu2_event_id");
                });
        }
        ...
        if(isset($vars['client_id'])){
            $db = $db->whereRaw("a.client_id='".$vars['client_id']."'");
        }
        if(isset($vars['srcEvt'])){
            $db = $db->where(function($query) use ($vars){
                ...
                $query = $query->orWhereRaw("a.desc LIKE '%".$vars['srcEvt']."%'");
            ...
            if(count($vars['orderBy']) > 0){
                foreach ($vars['orderBy'] as $k => $v) {
                    if($e = explode(":",$v)){
                        ...
                        if($field == 'ST'){
                            if(isset($vars['countPatient']) && $vars['countPatient']){
                                $db = $db->orderByRaw("IF(f.patient_total = 0, 0, IF(f.patient_total = (f.patient_canceled + f.patient_finished),2,1)) ".$order);
                            }
                        }else{
