...
class MCU2FormatTeam extends Model{
    use HasFactory;

    public static function query($vars = [], $params = []){
        $db =
            DB::table("mcu2_format_team AS a");
        ...
        if(isset($vars['srcEvt'])){
            $db = $db->where(function($query) use ($vars){
                ...
                $query = $query->orWhereRaw("a.str_number LIKE '%".$vars['srcEvt']."%'");
                $query = $query->orWhereRaw("a.sip_number LIKE '%".$vars['srcEvt']."%'");
            });
        }
