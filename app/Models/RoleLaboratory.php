    ...
    public static function query($vars = [], $params = []){
        ...
        if(isset($vars['srcEvt'])){
            $db = $db->where(function($query) use ($vars){
                $query = $query->orWhereRaw("c.f_role_name LIKE '%".$vars['srcEvt']."%'");
                $query = $query->orWhereRaw("b.f_role_desc LIKE '%".$vars['srcEvt']."%'");
            });
        }
        ...
        if(isset($vars['orderBy'])){
            ...
            $db = $db->orderBy('c.f_role_name', 'asc');
        }
