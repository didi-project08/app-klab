...
class RoleAccountCorporate extends Model{
    use HasFactory;

    public static function query($vars = [], $params = []){
        $db =
            DB::table("tc_role AS a")
            ->select(DB::raw("a.*, c.corporate_id, ca.code AS corporate_code, ca.title AS corporate_name"))
            ->join("role_corporate AS c", function($join){
