...
class MCU2FormatPackageItem extends Model{
    use HasFactory;

    public static function query($vars = [], $params = []){
        $db =
            DB::table("mcu2_format_package_item AS a")
            ->select(DB::raw("b.*, a.mcu_format_package_id"))
            ->join("mcu2_format_item AS b", function($join){
                $join->on("a.mcu_format_item_id","=","b.id");
            });
        
        if(isset($vars['delete'])){
            if($vars['delete'] == 0){
                $db = $db->whereRaw("b.delete_at IS NULL");
            }else if($vars['delete'] == 1){
                $db = $db->whereRaw("b.delete_at IS NOT NULL");
            }
        }else{
            $db = $db->whereRaw("b.delete_at IS NULL");
        }
        ...
        if(isset($vars['mcu_format_package_id'])){
            $db = $db->whereRaw("a.mcu_format_package_id='".$vars['mcu_format_package_id']."'");
        }
        if(isset($vars['mcu_format_item_id'])){
            $db = $db->whereRaw("a.mcu_format_item_id='".$vars['mcu_format_item_id']."'");
        }
        if(isset($vars['resultCate'])){
            $db = $db->whereRaw("b.main_parent='".$vars['resultCate']."'");
        }
        ...
        if(isset($vars['orderBy'])){
            ...
            $db = $db->orderBy('b.level', 'asc');
            $db = $db->orderBy('b.sort', 'asc');
        }
