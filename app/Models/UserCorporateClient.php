...
class UserCorporateClient extends Model{
    use HasFactory;

    public static function query($vars = [], $params = []){
        $db =
            DB::table("user_corporate_client AS a")
            ->select(DB::raw("b.*"))
            ->join("corporate_client AS b", function($join){
                $join->on("a.corporate_client_id","=","b.id");
            })
            ->groupBy("a.corporate_client_id");
