...
class UserAccountLaboratory extends Model{
    use HasFactory;

    public static function query($vars = [], $params = []){
        $db =
            DB::table("tc_user AS a")
            ->select(DB::raw("a.*, b.f_role_name, c.laboratory_id, ca.code AS laboratory_code, ca.title AS laboratory_name"))
            ->join("tc_role AS b", function($join){
                $join->on("a.f_role_id","=","b.f_role_id");
            })
            ->join("user_laboratory AS c", function($join){
