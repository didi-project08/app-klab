...
class MCUEventPatient extends Model{
    use HasFactory;
    ...
    public static function query($vars = [], $params = []){
            ...
            if(count($vars['userCorporate']) > 0){
                $db = $db->where(function($query) use ($vars){
                    foreach ($vars['userCorporate'] as $k => $v) {
                        $query = $query->orWhereRaw("b.corporate_id=".$v->id);
                    }
                });
            }else{
                $db = $db->whereRaw("a.id=-9");
            ...
            if(count($vars['userCorporateClient']) > 0){
                $db = $db->where(function($query) use ($vars){
                    foreach ($vars['userCorporateClient'] as $k => $v) {
                        $query = $query->orWhereRaw("b.corporate_client_id=".$v->id);
                    }
                });
            }else{
                $db = $db->whereRaw("a.id=-9");
            ...
            if(count($vars['userLaboratory']) > 0){
                $db = $db->where(function($query) use ($vars){
                    foreach ($vars['userLaboratory'] as $k => $v) {
                        $query = $query->orWhereRaw("b.laboratory_id=".$v->id);
                    }
                });
            }else{
                $db = $db->whereRaw("a.id=-9");
        ...
        if($params['mode'] == 'total'){
            ...
            foreach ($db as $k => $v) {
                ...
                $v->fit_date_dmY_slash = $v->fit_date;
                if(strtotime($v->fit_date)){
                    $v->fit_date_dmY_slash = date("d/m/Y", strtotime($v->fit_date));
                }
