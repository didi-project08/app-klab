...
class RoleAccountLaboratory extends Model{
    use HasFactory;

    public static function query($vars = [], $params = []){
        $db =
            DB::table("tc_role AS a")
            ->select(DB::raw("a.*, c.laboratory_id, ca.code AS laboratory_code, ca.title AS laboratory_name"))
            ->join("role_laboratory AS c", function($join){
                $join->on("a.f_role_id","=","c.f_role_id");
            })
            ->join("laboratory AS ca", function($join){
                $join->on("c.laboratory_id","=","ca.id");
            })
            ->groupBy("a.f_role_id");
        ...
        if(isset($vars['f_role_type'])){
            $db = $db->whereRaw("a.f_role_type='".$vars['f_role_type']."'");
        }
            ...
            if(count($vars['userLaboratory']) > 0){
                $db = $db->where(function($query) use ($vars){
                    foreach ($vars['userLaboratory'] as $k => $v) {
                        $query = $query->orWhereRaw("c.laboratory_id=".$v->id);
                    }
                });
            }else{
        ...
        if(isset($vars['srcEvt'])){
            $db = $db->where(function($query) use ($vars){
                $query = $query->orWhereRaw("a.f_role_name LIKE '%".$vars['srcEvt']."%'");
                $query = $query->orWhereRaw("a.f_role_desc LIKE '%".$vars['srcEvt']."%'");
