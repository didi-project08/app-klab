...
class MCU2FormatRefItem extends Model{
    use HasFactory;

    public static function query($vars = [], $params = []){
        $db =
            DB::table("mcu2_format_ref_item AS a")
            ->select(DB::raw("a.*, b.code AS mcu2_format_ref_code"))
            ->join("mcu2_format_ref AS b", function($join){
                $join->on("a.mcu2_format_ref_id","=","b.id");
            });
        ...
        if(isset($vars['mcu2_format_ref_id'])){
            $db = $db->whereRaw("a.mcu2_format_ref_id='".$vars['mcu2_format_ref_id']."'");
        }
        if(isset($vars['item_code'])){
            $db = $db->whereRaw("a.item_code='".$vars['item_code']."'");
        }

        if(isset($vars['mcu_format_id'])){
            $db = $db->whereRaw("b.mcu_format_id='".$vars['mcu_format_id']."'");
        }

        if(isset($vars['srcEvt'])){
            $db = $db->where(function($query) use ($vars){
                $query = $query->orWhereRaw("a.item_code LIKE '%".$vars['srcEvt']."%'");
                $query = $query->orWhereRaw("a.type LIKE '%".$vars['srcEvt']."%'");
                $query = $query->orWhereRaw("a.from LIKE '%".$vars['srcEvt']."%'");
                $query = $query->orWhereRaw("a.to LIKE '%".$vars['srcEvt']."%'");
                $query = $query->orWhereRaw("a.unit LIKE '%".$vars['srcEvt']."%'");
            });
        }
