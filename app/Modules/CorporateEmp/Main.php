<?php
namespace App\Modules\CorporateEmp;
...
class Main extends MY_Controller{
    ...
    public function __construct(){
        parent::__construct();
        $this->_viewPath = "CorporateEmp/";
        $this->_vars["_id_"] = "M41";
        $this->_vars['url'] = URL::to('/')."/corporate-emp/";
    }
    ...
    public function read(){
        ...
        $this->_vars['orderBy'] = ['nextMCU:asc'];
    ...
    public function create(Request $request){
        ...
        if ($validator->fails()){
            ...
            try{
                ...
                DB::table('corporate_emp')->insert($insert);
    ...
    public function edit(Request $request, $id = 0){
        $this->_vars['id'] = $id;
        $sel = CorporateEmp::query($this->_vars, ["mode"=>"rowOne"]);
        if($sel['success']){
    ...
    public function update(Request $request, $id = 0){
        ...
        if ($validator->fails()){
            ...
            try{
                ...
                DB::table('corporate_emp')->where($where)->update($update);
    ...
    public function export(Request $request){
        $this->_vars['userCorporate'] = ModelBase::UserCorporate();
        $contentList = CorporateEmp::query($this->_vars, ["mode"=>"rowsAll"]);
        $fieldList = CorporateEmp::excelField();
        ...
        $xr = 1; $xc = 0; $xc++;
        $c->setCellValue(MainHelper::getCol($xc).$xr, 'Corporate Employee');
        ...
        $fileName = "CorporateEmployee_".uniqid().".xlsx";
    ...
    public function importTemplate(){
        $fieldList = CorporateEmp::importTemplateField();
        ...
        $fileName = "EmployeeImportTempate.xlsx";
        header('Content-Type: application/vnd.openxmlformats-officedocument.spreadsheetml.sheet');
        header('Content-Disposition: attachment;filename="'.$fileName.'"');
        header('Cache-Control: max-age=0');
                    ...
                    if(isset($sheetData[0][1]) && $sheetData[0][1] == md5(env('SYS_IMPORT_TEMPLATE_CODE'))){
                        ...
                        foreach ($sheetData as $k => $v) {
                            if($k >= 3){
                                ...
                                $insert['gender'] = $v[3];
                                $insert['dob'] = $v[4];
                                // $insert['dob'] = \PhpOffice\PhpSpreadsheet\Shared\Date::excelToDateTimeObject($v[4])->format("Y-m-d");
                                $insert['phone'] = $v[5];
                                $insert['address'] = $v[6];
                                $insert['area'] = $v[7];
                                $insert['division'] = $v[8];
                                ...
                                $validator = Facades\Validator::make($insert, [
                                    ...
                                    'gender.in' => "Please type a valid :attribute (M/F)",
                                    // 'dob.date_format' => "The :attribute does not match the format dd/mm/yyyy."
                                ])->setAttributeNames([
                                    'emp_no' => 'Emp. Number',
                                    'area' => 'Job Area',
                                ...
                                $validator->after(function (Validator $validator) use ($insert) {
                                    if(!is_numeric($insert['dob'])){
                                        $validator->errors()->add('dob', 'DOB is not valid.');
                                    }
                                });
                                if ($validator->fails()){
                                    $excelRowValidation[] = [
                                    ...
                                    $insert['dob'] = \PhpOffice\PhpSpreadsheet\Shared\Date::excelToDateTimeObject($insert['dob'])->format("Y-m-d");
