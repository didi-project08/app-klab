...
<cpModalTitle>Confirm MCU Project</cpModalTitle>
<cpModalFooter>
    <button class="btn btn-outline-danger" type="button" onclick="cpGlobalModalClose('{{$cpModalId}}')">Close</button>
    <button class="btn btn-outline-primary" type="button" onclick="jf{{$_idb_}}dGridSave()">Confirm</button>
</cpModalFooter>

<div id="cp{{$_idb_}}dGridFormConfirmContent">
    <form id="cp{{$_idb_}}dGridFormConfirm" method="post" enctype="multipart/form-data">
        @csrf
        @method('PUT')
        <div class="row">
            <div class="col-12 col-md-12 mb-3">
                <select class="" id="i{{$_idb_}}status" name="status" style="width:100%"></select>
                <div id="status_error" class='form_error'></div>
            </div>
            <div class="col-12 col-md-12 mb-3">
                <select class="" id="i{{$_idb_}}confirm_note" name="confirm_note" style="width:100%"></select>
...
<script>
    $eu(function(){
        ...
        $eu('#i{{$_idb_}}confirm_note').textbox({
            required:true,
            label:"Note",
            labelPosition:'top',
            prompt:"Note",
    ...
    function jf{{$_idb_}}dGridSave(){
        preloader_block();
        $eu("#cp{{$_idb_}}dGridFormConfirm").form("submit", {
            url: "{{ @$url_save }}",
            onSubmit: function() {
