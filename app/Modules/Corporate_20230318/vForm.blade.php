    ...
    <form id="cp{{$_id_}}dGridForm" method="post" enctype="multipart/form-data">
        ...
        <div class="row">
            <div class="col-12 col-md-12 mb-3">
                <input class="form-controls" type="text" id="laboratory_id" name="laboratory_id" style="width:100%">
...
<?php if(@$mode == 'add') { ?>
    ...
    <script>
        $eu(function(){
            $eu('#laboratory_id').combobox("reload",'{{$url}}laboratoryCombo?selectFirst=1');
        })
    </script>
<?php } else if(@$mode == 'edit') { ?>
    <script>
        $(function(){
            $eu('#laboratory_id').combobox("reload",'{{$url}}laboratoryCombo');
            var selData = {!! $selData !!};
            $eu("#cp{{$_id_}}dGridForm").form('load', selData);
            $eu('#laboratory_id').combobox('disable',true);
