...
class B2Main extends MY_Controller{
    public function __construct(){
        parent::__construct();
        $this->_viewPath = "Provider/";
        $this->_vars["_id_"] = "M32";
        $this->_vars["_idb_"] = $this->_vars["_id_"]."b2";
        $this->_vars['urlb'] = URL::to('/')."/provider/";
        $this->_vars['urlb'] = $this->_vars['urlb']."b2/";
    }
    public function index(){
        $this->_vars['userScope'] = ModelBase::m_user_scope();
        ModelBase::ShowPage($this->_viewPath.'v2Main', $this->_vars);
    }
    ...
    public function read(){
        ...
        $result['rows'] = Provider::query($this->_vars, ["mode"=>"rows"]);
        $result['total'] = Provider::query($this->_vars, ["mode"=>"total"]);
    ...
    public function create(Request $request){
        $validator = Facades\Validator::make($request->all(), [
            ...
            'pic_email' => ['nullable','string','max:100']
        ])->setAttributeNames([
            'type_id' => 'Provider Type',
            'title' => 'Provider Name',
            'prov' => 'Province',
            'city' => 'City',
            'address' => 'Address',
        ...
        if ($validator->fails()){
            ...
            try{
                ...
                $insert['create_at'] = $at;
                DB::table('provider')->insert($insert);
    ...
    public function edit(Request $request, $id = 0){
        $this->_vars['id'] = $id;
        $sel = Provider::query($this->_vars, ["mode"=>"rowOne"]);
        if($sel['success']){
    ...
    public function update(Request $request, $id = 0){
        ...
        if ($validator->fails()){
            ...
            try{
                ...
                DB::table('provider')->where($where)->update($update);
    ...
    public function export(Request $request){
        $contentList = Provider::query($this->_vars, ["mode"=>"rowsAll"]);
        $fieldList = Provider::excelField();
        ...
        $xr = 1; $xc = 0; $xc++;
        $c->setCellValue(MainHelper::getCol($xc).$xr, 'Provider');
        ...
        $fileName = "Provider_".uniqid().".xlsx";
    ...
    public function importTemplate(){
        $fieldList = Provider::importTemplateField();
        ...
        $fileName = "ProviderImportTempate.xlsx";
        header('Content-Type: application/vnd.openxmlformats-officedocument.spreadsheetml.sheet');
        header('Content-Disposition: attachment;filename="'.$fileName.'"');
        header('Cache-Control: max-age=0');
