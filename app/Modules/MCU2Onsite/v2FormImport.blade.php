<?php if(@$mode == 'add') { ?>
    ...
    <cpModalFooter>
        <button class="btn btn-outline-primary" type="button" onclick="jf{{$_idb_}}dGridImportTemplate()">TEMPLATE</button>|
    ...
    <form id="cp{{$_idb_}}dGridFormImport" method="post" enctype="multipart/form-data">
        ...
        <div class="row">
            <div class="col-12 col-md-12 mb-3">
                <input class="" type="text" id="cp{{$_idb_}}filePatient" name="file" style="width:100%">
...
<script>
    $eu(function(){
        $eu('#cp{{$_idb_}}filePatient').filebox({
            required:true,
    ...
    function jf{{$_idb_}}dGridImportTemplate(){
        window.open("{{ $urlb }}patientImportTemplate");
    }
...
<?php } else { ?>
    <script>
        $(function(){
            $("#cp{{$_idb_}}dGridFormImportContent").html("Request not valid.");
        })
    </script>
<?php } ?>