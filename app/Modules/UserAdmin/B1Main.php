<?php
namespace App\Modules\UserAdmin;
...
class B1Main extends MY_Controller{
    ...
    public function __construct(){
        parent::__construct();
        $this->_viewPath = "UserAdmin/";
        $this->_vars["_id_"] = "M11";
        $this->_vars["_idb_"] = $this->_vars["_id_"]."b1";
        $this->_vars['url'] = URL::to('/')."/user-admin/";
    ...
    public function read(){
        $qVars['delete'] = -1;
        $this->_vars['f_user_type'] = 1;
        $result['rows'] = UserAccount::query($this->_vars, ["mode"=>"rows"]);
        $result['total'] = UserAccount::query($this->_vars, ["mode"=>"total"]);
    ...
    public function create(Request $request){
        ...
        if ($validator->fails()){
            ...
            try{
                $insert = [];
                $insert['f_user_type'] = 1;
                $insert['f_user_admin'] = 1;
                $insert['f_active'] = 1;
    ...
    public function edit(Request $request, $id = 0){
        ...
        $this->_vars['f_user_type'] = 1;
        $sel = UserAccount::query($this->_vars, ["mode"=>"rowOne"]);
        if($sel['success']){
