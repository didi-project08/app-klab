<?php
namespace App\Modules\Laboratory;
...
class Main extends MY_Controller{
    ...
    public function __construct(){
        parent::__construct();
        $this->_viewPath = "Laboratory/";
        $this->_vars["_id_"] = "M4";
        $this->_vars['url'] = URL::to('/')."/laboratory/";
    }
    ...
    public function read(){
        $result['rows'] = Laboratory::query($this->_vars, ["mode"=>"rows"]);
        $result['total'] = Laboratory::query($this->_vars, ["mode"=>"total"]);
    ...
    public function create(Request $request){
        ...
        $validator->after(function (Validator $validator) use ($request) {
            $db = 
                DB::table(DB::raw("laboratory AS a"))
                ->whereRaw("a.code='".$request->post('code')."'")
                ->limit(1);
            $db = $db->get();
            $total = count($db);
            if($total > 0){
                $validator->errors()->add('code', 'Provider Code already exist, try another one.');
            }
        });
        ...
        if ($validator->fails()){
            ...
            try{
                ...
                $insert['create_at'] = $at;
                DB::table('laboratory')->insert($insert);
    ...
    public function edit(Request $request, $id = 0){
        $this->_vars['id'] = $id;
        $sel = Laboratory::query($this->_vars, ["mode"=>"rowOne"]);
        if($sel['success']){
    ...
    public function update(Request $request, $id = 0){
        ...
        if ($validator->fails()){
            ...
            try{
                ...
                DB::table('laboratory')->where($where)->update($update);
    ...
    public function export(Request $request){
        $contentList = Laboratory::query($this->_vars, ["mode"=>"rowsAll"]);
        $fieldList = Laboratory::excelField();
    ...
    public function importTemplate(){
        $fieldList = Laboratory::importTemplateField();
