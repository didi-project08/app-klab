<?php if(@$mode == 'add') { ?>
    ...
    <cpModalFooter>
        ...
        <button class="btn btn-outline-primary" type="button" onclick="jf{{$_idbf_}}dGridSave(1)">Submit & Create New</button>
    </cpModalFooter>
    ...
    <form id="cp{{$_idbf_}}dGridForm" method="post" enctype="multipart/form-data">
        ...
        <div class="row">
            <div class="col-12 col-md-12 mb-3">
                <input class="form-controls" type="text" id="i{{$_idbf_}}title" name="title" style="width:100%">
                <div id="title_error" class='form_error'></div>
            </div>
            <div class="col-12 col-md-6 mb-3">
                <input class="form-controls" type="text" id="i{{$_idbf_}}price_lab" name="price_lab" style="width:100%; text-align:right">
                <div id="price_lab_error" class='form_error'></div>
            </div>
            <div class="col-12 col-md-6 mb-3">
                <input class="form-controls" type="text" id="i{{$_idbf_}}price_corporate" name="price_corporate" style="width:100%; text-align:right">
...
<script>
    $eu(function(){
        $eu('#i{{$_idbf_}}title').textbox({
            required:true,
            label:"Package Name",
            labelPosition:'top',
            prompt:"Package Name"
        });
        $eu('#i{{$_idbf_}}price_lab').numberbox({
            required:true,
        ...
        $eu('#i{{$_idbf_}}price_corporate').numberbox({
            required:true,
        ...
        setTimeout(() => {
            $eu('#i{{$_idbf_}}title').textbox('textbox').focus();
        }, 500);
    })

    function jf{{$_idbf_}}dGridSave(createNew = 0){
        preloader_block();
