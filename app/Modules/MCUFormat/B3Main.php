...
class B3Main extends MY_Controller{
    ...
    public function read(){
        ...
        $result['rows'] = MCUFormatPackage::query($this->_vars, ["mode"=>"rows"]);
        $result['total'] = MCUFormatPackage::query($this->_vars, ["mode"=>"total"]);
        echo json_encode($result);
    }
    ...
    public function create(Request $request, $mcu_format_id = 0){
        $validator = Facades\Validator::make($request->all(), [
            ...
            'price_lab' => ['required','numeric'],
            'price_corporate' => ['required','numeric'],
        ])->setAttributeNames([
            'title' => 'Package Name',
        ...
        if ($validator->fails()){
            ...
            try{
                ...
                DB::table('mcu_format_package')->insert($insert);
    ...
    public function edit(Request $request, $id = 0){
        $this->_vars["_idbf_"] = $this->_vars["_idb_"]."f1";
        $this->_vars['id'] = $id;
        $sel = MCUFormatPackage::query($this->_vars, ["mode"=>"rowOne"]);
        if($sel['success']){
    ...
    public function update(Request $request, $id = 0){
        ...
        if ($validator->fails()){
            ...
            try{
                ...
                DB::table('mcu_format_package')->where($where)->update($update);
