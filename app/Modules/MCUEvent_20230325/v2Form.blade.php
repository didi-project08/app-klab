<?php if(@$mode == 'add') { ?>
    ...
    <cpModalFooter>
        ...
        <button class="btn btn-outline-primary" type="button" onclick="jf{{$_id_}}b2dGridSave()">Create</button>
...
<?php } else if(@$mode == 'edit') { ?>
    ...
    <cpModalFooter>
        <button class="btn btn-outline-danger" type="button" onclick="cpGlobalModalClose('{{$cpModalId}}')">Close</button>
        <button class="btn btn-outline-primary" type="button" onclick="jf{{$_id_}}b2dGridSave()">Update</button>
    </cpModalFooter>
<?php } else if(@$mode == 'present') { ?>
    ...
    <cpModalFooter>
        <button class="btn btn-outline-danger" type="button" onclick="cpGlobalModalClose('{{$cpModalId}}')">Close</button>
        <button class="btn btn-outline-primary" type="button" onclick="jf{{$_id_}}b2dGridSave()">Present</button>
    </cpModalFooter>
<?php } ?>

<div id="cp{{$_id_}}b2dGridFormContent">
    <form id="cp{{$_id_}}b2dGridForm" method="post" enctype="multipart/form-data">
        @csrf
        ...
        <div class="col-12 col-md-12 mb-3">
                <select class="" id="type" name="type" style="width:100%"></select>
                <div id="type_error" class='form_error'></div>
            </div>
            <div class="col-12 col-md-12 mb-3" id="corporate_id_frame">
                <select class="" id="corporate_id" name="corporate_id" style="width:100%"></select>
                <div id="corporate_id_error" class='form_error'></div>
            </div>
            <div class="col-12 col-md-12 mb-3" id="corporate_emp_id_frame">
                <select class="" id="corporate_emp_id" name="corporate_emp_id" style="width:100%"></select>
                <div id="corporate_emp_id_error" class='form_error'></div>
            </div>
        ...
        <div class="row">
            ...
            <div class="col-12 col-md-6 mb-3">
                <input class="" type="text" id="mcu_format_id" name="mcu_format_id" style="width:100%">
                <div id="mcu_format_id_error" class='form_error'></div>
            </div>
            <div class="col-12 col-md-6 mb-3">
                <input class="" type="text" id="mcu_format_package_id" name="mcu_format_package_id" style="width:100%">
...
<script>
    $eu(function(){
        ...
        $eu('#corporate_id').combobox({
            ...
            method: "GET",
            url:'{{$url}}corporateCombo?laboratory_id={{ $eventData->laboratory_id }}',
            editable:true,
            valueField:'id',
            textField:'title',
            panelHeight:150
        ...
        $eu("#corporate_emp_id").combogrid({
            ...
            onHidePanel:function(){
                ...
                if(sel){
                    preloader_block()
                    setTimeout(() => {
                        $eu("#cp{{$_id_}}b2dGridForm").form('load', sel);
                        preloader_none();
                    }, 500);
                }
            }
        ...
        //     method: "GET",
        //     url:'{{$url}}laboratoryCombo',
        //     editable:false,
        //     valueField:'id',
        //     textField:'title',
        //     panelHeight:150,
        //     value:"{{ $eventData->laboratory_id }}"
        // });
    ...
    function jf{{$_id_}}b2dGridSave(){
        preloader_block();
        $eu("#cp{{$_id_}}b2dGridForm").form("submit", {
            url: "{{ @$url_save }}",
            onSubmit: function() {
...
<?php if(@$mode == 'add') { ?>
    ...
    <script>
        $(function(){
            $eu('#corporate_id').combobox({
                onChange:function(newValue,oldValue){
                    $eu("#corporate_emp_id").combogrid({url:'{{$url}}readCorporateEmp?corporate_id='+newValue})
                }
            });
            setTimeout(() => {
...
<?php } else if(@$mode == 'edit' || @$mode == 'present') { ?>
    <script>
        $(function(){
            ...
            $eu('#corporate_id').combobox({
                onChange:function(newValue,oldValue){
                    $eu("#corporate_emp_id").combogrid({url:'{{$url}}readCorporateEmp?id='+selData.corporate_emp_id})
                }
            });
            $eu("#cp{{$_id_}}b2dGridForm").form('load', selData);
...
<?php } else { ?>
    <script>
        $(function(){
            $("#cp{{$_id_}}b2dGridFormContent").html("Request not valid.");
        })
    </script>
<?php } ?>