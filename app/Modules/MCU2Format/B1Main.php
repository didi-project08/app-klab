...
class B1Main extends MY_Controller{
    ...
    public function read(){
        $result['rows'] = MCU2Format::query($this->_vars, ["mode"=>"rows"]);
        $result['total'] = MCU2Format::query($this->_vars, ["mode"=>"total"]);
        echo json_encode($result);
    }
    public function add(){
        $this->_vars['mode'] = 'add';
    ...
    public function create(Request $request){
        ...
        if ($validator->fails()){
            ...
            try{
                ...
                $insert['create_at'] = $at;
                DB::table('mcu2_format')->insert($insert);
    ...
    public function edit(Request $request, $id = 0){
        $this->_vars['id'] = $id;
        $sel = MCU2Format::query($this->_vars, ["mode"=>"rowOne"]);
        if($sel['success']){
    ...
    public function update(Request $request, $id = 0){
        ...
        if ($validator->fails()){
            ...
            try{
                ...
                DB::table('mcu2_format')->where($where)->update($update);
