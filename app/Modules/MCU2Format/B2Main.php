...
class B2Main extends MY_Controller{
    ...
    //     $result['rows'] = MCU2FormatItem::query($this->_vars, ["mode"=>"rows"]);
    //     $result['total'] = MCU2FormatItem::query($this->_vars, ["mode"=>"total"]);
    //     echo json_encode($result);
    // }
    public function read(){
        $this->_vars['orderBy'] = ["a.level:asc","a.sort:asc","a.id:asc"];
    ...
    public function add(Request $request, $mcu_format_id = 0, $id = 0){
        ...
        $sel = MCU2Format::query($qVars, ["mode"=>"rowOne"]);
        if($sel['success']){
            $qVars['id'] = $id;
            $sel2 = MCU2FormatItem::query($qVars, ["mode"=>"rowOne"]);
    ...
    public function createInput(Request $request, $mcu_format_id = 0, $id = 0){
        $validator = Facades\Validator::make($request->all(), [
            ...
            'input_default' => ['nullable','string'],
            'input_option' => ['nullable','string'],
            'input_desc' => ['required','integer'],
            ...
            'input_default' => 'Defaule Value',
            'input_option' => 'Input Option',
            'input_desc' => 'Description Box',
            'unit' => 'Unit',
    ...
    public function edit(Request $request, $id = 0){
        $this->_vars["_idbf_"] = $this->_vars["_idb_"]."f1";
        $qVars['id'] = $id;
        $sel = MCU2FormatItem::query($qVars, ["mode"=>"rowOne"]);
        if($sel['success']){
        ...
        if($sel['success']){
            ...
            if ($validator->fails()){
                ...
                try{
                    ...
                    DB::table('mcu2_format_item')->where($where)->update($update);
    ...
    public function updateInput(Request $request, $id = 0){
        ...
        if($sel['success']){
            ...
            $validator->after(function (Validator $validator) use ($request, $id, $sel) {
                // no addtional validation
                $db = 
                    DB::table(DB::raw("mcu2_format_item AS a"))
                    ->whereRaw("a.id != '".$id."'")
        ...
        if ($validator->fails()){
            ...
            try{
                ...
                if($requestAll['moveTo'] == 1){
                    DB::table('mcu2_format_item')->where(["id"=>$requestAll['centerId']])->update(["sort"=>$requestAll['beforeSort']]);
                    DB::table('mcu2_format_item')->where(["id"=>$requestAll['beforeId']])->update(["sort"=>$requestAll['centerSort']]);
                }else{
                    DB::table('mcu2_format_item')->where(["id"=>$requestAll['centerId']])->update(["sort"=>$requestAll['afterSort']]);
                    DB::table('mcu2_format_item')->where(["id"=>$requestAll['afterId']])->update(["sort"=>$requestAll['centerSort']]);
                }
                DB::commit();
                $result['success'] = true;
    ...
    public function duplicateToSave(Request $request, $id = 0){
        if($id != $request->all()['idTo']){
            ...
            if($sel['success']){
                ...
                $sel2 = MCU2Format::query($qVars, ["mode"=>"rowOne"]);
                if($sel['success']){
                    $nextProcess = true;
                }else{
                    $result['success'] = false;
        ...
        if($nextProcess = true){
            ...
            if ($validator->fails()){
                ...
                try{
                    ...
                    $mcuFormat = MCU2FormatItem::query($qVars, ["mode"=>"rowsAll"]);
                    $mcuFormatTree = MCU2FormatItem::converToTree($mcuFormat, @$mcuFormat[0]->level);
                    ...
                    if(count($mcuFormatTree) > 0){
                        DB::table("mcu2_format_item")->where(["mcu_format_id"=>$request->all()['idTo']])->delete();
                        MCU2FormatItem::duplicate($mcuFormatTree, $request->all()['idTo']);
