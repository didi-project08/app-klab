...
<div id="cp{{$_idb_}}dGridFormContent">
    <form id="cp{{$_idb_}}dGridForm" method="post" enctype="multipart/form-data">
        ...
        <div class="row">
            ...
            <div class="col-12 col-md-12 mb-3">
                <select class="" id="i{{$_idb_}}f_role_id" name="f_role_id" style="width:100%"></select>
                <div id="f_role_id_error" class='form_error'></div>
            </div>
            <div class="col-12 col-md-12 mb-3">
                <input class="form-controls" type="text" id="i{{$_idb_}}f_username" name="f_username" style="width:100%">
                <div id="f_username_error" class='form_error'></div>
            </div>
            <div class="col-12 col-md-12 mb-3">
                <input class="form-controls" type="text" id="i{{$_idb_}}f_password" name="f_password" style="width:100%">
...
<script>
    $eu(function(){
        ...
        setTimeout(() => {
            $eu('#i{{$_idb_}}f_fullname').textbox('textbox').focus();
        }, 500);
        $eu('#i{{$_idb_}}f_username').textbox({
            required:true,
            label:"Username",
            labelPosition:'top',
            prompt:"Username"
        });
        $eu('#i{{$_idb_}}f_password').passwordbox({
            required:true,
            label:"Password",
            labelPosition:'top',
            prompt:"Password"
        });
        $eu('#i{{$_idb_}}f_role_id').combobox({
            required:true,
            label:"Role",
            labelPosition:'top',
            prompt:"Choose Role",
        ...
        $eu(function(){
            $eu('#i{{$_idb_}}corporate_id').combobox({
                ...
                onChange:function(newValue,oldValue){
                    $eu('#i{{$_idb_}}f_role_id').combobox('reload','{{$urlb}}readRoleCombo?corporate_id='+newValue);
                }
            });
            $eu('#i{{$_idb_}}f_role_id').combobox({
                onLoadSuccess:function(){
                    $eu('#i{{$_idb_}}f_role_id').combobox('setValue','');
                }
            });
        })
    </script>
<?php } else if(@$mode == 'edit') { ?>
    <script>
        $eu(function(){
            ...
            $eu('#i{{$_idb_}}f_username').textbox('disable',true);
        })
    </script>
<?php } else { ?>
    <script>
