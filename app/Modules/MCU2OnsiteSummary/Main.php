...
class Main extends MY_Controller{
    ...
    public function readClientCombo(){
        $this->_vars['orderBy'] = ['a.title:asc'];
        $result = Provider::query($this->_vars, ["mode"=>"rowsAll"]);
    ...
    public function readMcu2FormatCombo(){
        $this->_vars['orderBy'] = ['a.title:asc'];
    ...
    public function readMcu2FormatPackageCombo(){
        $this->_vars['orderBy'] = ['a.code:asc'];
        $this->_vars['orderBy'] = ['a.title:asc'];
        $result = MCU2FormatPackage::query($this->_vars, ["mode"=>"rowsAll"]);
        if(isset($this->_vars['selectFirst']) && $this->_vars['selectFirst'] == 1){
            if(count($result) > 0){
                $result[0]->selected = true;
            }
    ...
    public function mcuItemLv0Combo(Request $request, $mcu_format_id = 0){
        $this->_vars['mcu_format_id'] = $mcu_format_id;
        $this->_vars['orderBy'] = ["a.sort:asc"];
        $result = MCU2FormatItem::query($this->_vars, ["mode"=>"rowsAll"]);

        $data = [];
        $v = (object) ["id"=>-1, "name"=>"-- ALL --"];
        array_push($data, $v);
        $v = (object) ["id"=>0, "name"=>"COVER"];
        array_push($data, $v);
